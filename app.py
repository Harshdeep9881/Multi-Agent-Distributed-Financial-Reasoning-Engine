...
"""
            ...
            for symbol, exp_data in exposure.items():
                ...
                currency = exp_data.get('currency', currency_for_symbol(symbol))
                brief += f"\n- **{company_name} ({symbol})**: {weight_pct:.1f}% allocation (${value:,.0f}) @ {price:,.2f} {currency}"
...
def start_fastapi_once():
    # Streamlit reruns the script on every interaction; avoid spawning multiple servers.
    if _is_port_in_use(8000):
        return
    if not st.session_state.get("fastapi_started", False):
                                                ...
                                                with comparison_cols[idx]:
                                                    ...
                                                    if symbol in market_data:
                                                        ...
                                                        if isinstance(stock_data, dict):
                                                            st.metric(
                                                                "Current Price",
                                                                f"{stock_data.get('price', 'N/A'):,.2f} {currency_for_symbol(symbol)}" if isinstance(stock_data.get('price'), (int, float)) else stock_data.get('price', 'N/A')
                                                            )
                                                            ...
                                                            if stock_data.get('52_week_high'):
                                                                st.caption(f"52W High: {stock_data.get('52_week_high', 'N/A')} {currency_for_symbol(symbol)}")
                                                            
                                                            if stock_data.get('52_week_low'):
                                                                st.caption(f"52W Low: {stock_data.get('52_week_low', 'N/A')} {currency_for_symbol(symbol)}")
                                        ...
                                        for symbol in selected_symbols:
                                            if symbol in market_data:
                                                ...
                                                if isinstance(stock_data, list) and len(stock_data) > 0:
                                                    try:
                                                        ...
                                                        comparison_data[symbol] = {
                                                            'Current Price': f"{latest.get('Close', 'N/A'):,.2f} {currency_for_symbol(symbol)}" if 'Close' in latest and pd.notna(latest.get('Close')) else 'N/A',
                                                            ...
                                                            '52W High': f"{df['High'].max():.2f} {currency_for_symbol(symbol)}" if 'High' in df.columns else 'N/A',
                                                            '52W Low': f"{df['Low'].min():.2f} {currency_for_symbol(symbol)}" if 'Low' in df.columns else 'N/A',
                                                ...
                                                # If stock_data is already a dict with metrics
                                                    ...
                                                    comparison_data[symbol] = {
                                                        'Current Price': f"{stock_data.get('price', 'N/A'):,.2f} {currency_for_symbol(symbol)}" if isinstance(stock_data.get('price'), (int, float)) else stock_data.get('price', 'N/A'),
                                                        'Daily Change %': f"{stock_data.get('change', 'N/A')}%",
                                                        '52W High': f"{stock_data.get('52_week_high', 'N/A')} {currency_for_symbol(symbol)}",
                                                        '52W Low': f"{stock_data.get('52_week_low', 'N/A')} {currency_for_symbol(symbol)}",
