...
# Lightweight in-memory caches (single-process)
...
def _cache_get(cache_store: dict, key):
...
def _cache_set(cache_store: dict, key, value, ttl_seconds: int):
    cache_store[key] = (time.time() + ttl_seconds, copy.deepcopy(value))


def _articles_fingerprint(articles: list) -> str:
    parts = []
    for article in articles:
        if isinstance(article, dict):
            parts.append(f"{article.get('title', '')}|{article.get('url', '')}")
        else:
            parts.append(str(article)[:120])
...
def extract_symbols_from_query(query):
    ...
    # Pull candidate tokens and filter to avoid matching normal words.
    tokens = re.findall(r'\b[A-Za-z0-9][A-Za-z0-9.-]{0,9}\b', query)
    direct_tickers = []
    for token in tokens:
        has_digit = any(ch.isdigit() for ch in token)
        has_sep = "." in token or "-" in token
        is_all_caps = token.isalpha() and token.isupper() and 1 <= len(token) <= 6
        if has_digit or has_sep or is_all_caps:
            direct_tickers.append(token.upper())
...
async def retrieve(
    ...
    try:
        ...
        market_data = None
        market_cache_key = tuple(symbol_list)
        cached_market_data, market_cache_hit = _cache_get(_market_data_cache, market_cache_key)
        if market_cache_hit:
            market_data = cached_market_data
            market_status = "cache_hit"
        else:
            market_data = api_agent.get_market_data(symbol_list)
            market_status = "success" if market_data else "failed"
            if market_data:
                _cache_set(_market_data_cache, market_cache_key, market_data, MARKET_DATA_TTL_SECONDS)
        market_time_ms = int((time.perf_counter() - market_start) * 1000)
        timing_breakdown_ms["market_data"] = market_time_ms
        agents_used.append({"name": "API Agent", "status": market_status, "time_ms": market_time_ms})
        ...
        if not market_data:
            ...
            timing_breakdown_ms["total_response_time"] = total_time_ms
            return {
        ...
        news_cache_key = tuple(symbol_list[:3])
        cached_articles, news_cache_hit = _cache_get(_news_cache, news_cache_key)
        if news_cache_hit:
            articles = cached_articles
            news_status = "cache_hit"
        else:
            articles = scraping_agent.scrape_news(news_urls, timeout=15)
            news_status = "success" if articles else "fallback_used"
        ...
        elif not news_cache_hit:
            logger.info(f"✅ Successfully scraped {len(articles)} real news articles")
        if not news_cache_hit:
            _cache_set(_news_cache, news_cache_key, articles, NEWS_TTL_SECONDS)
        news_time_ms = int((time.perf_counter() - news_start) * 1000)
        timing_breakdown_ms["news"] = news_time_ms
        agents_used.append({"name": "Scraping Agent", "status": news_status, "time_ms": news_time_ms})
        
        # Step 4: Index and retrieve
        retrieval_start = time.perf_counter()
        retrieval_cache_key = (query.strip().lower(), tuple(symbol_list), _articles_fingerprint(articles))
        cached_context, retrieval_cache_hit = _cache_get(_retrieval_cache, retrieval_cache_key)
        if retrieval_cache_hit:
            ...
            if context_docs:
                if hasattr(context_docs[0], 'page_content'):
                    ...
                    # Dictionary objects
                    context = [doc.get('page_content') or doc.get('text') or str(doc) for doc in context_docs]
                else:
                    # Direct text objects
            ...
            if not context or len(context) == 0:
                retrieval_status = "fallback_used"
            ...
            _cache_set(_retrieval_cache, retrieval_cache_key, context, RETRIEVAL_TTL_SECONDS)
        retrieval_time_ms = int((time.perf_counter() - retrieval_start) * 1000)
        timing_breakdown_ms["retrieval"] = retrieval_time_ms
        agents_used.append({"name": "Retriever Agent", "status": retrieval_status, "time_ms": retrieval_time_ms})
...
async def analyze(data: dict):
    try:
        ...
        upstream_agents = data.get("data", {}).get("agents_used", [])
        upstream_timing = data.get("data", {}).get("timing_breakdown_ms", {})
        ...
        if not exposure:
            ...
            for symbol in symbols:
                exposure[symbol] = {
                    ...
                    'currency': analysis_agent._currency_for_symbol(symbol)
                }
            logger.info("Using fallback exposure data")
        analysis_time_ms = int((time.perf_counter() - analysis_start) * 1000)
        timing_breakdown_ms["risk_analysis"] = analysis_time_ms
        agents_used.append({"name": "Analysis Agent", "status": analysis_status, "time_ms": analysis_time_ms})
        
        # Step 3: Get earnings
        earnings_start = time.perf_counter()
        earnings = {}
        earnings_status = "success"
        
        for symbol in symbols:
            try:
                earnings[symbol] = api_agent.get_earnings(symbol)
                if earnings[symbol] is None:
                    earnings_status = "fallback_used"
        ...
        earnings_time_ms = int((time.perf_counter() - earnings_start) * 1000)
        timing_breakdown_ms["earnings"] = earnings_time_ms
        agents_used.append({"name": "API Agent (Earnings)", "status": earnings_status, "time_ms": earnings_time_ms})
...
"""
        ...
        language_time_ms = int((time.perf_counter() - language_start) * 1000)
        timing_breakdown_ms["language"] = language_time_ms
        agents_used.append({"name": "Language Agent", "status": language_status, "time_ms": language_time_ms})
        
        logger.info("Analysis completed successfully")
        total_time_ms = int((time.perf_counter() - total_start) * 1000)
        timing_breakdown_ms["total_response_time"] = total_time_ms
        merged_agents = upstream_agents + agents_used
        merged_timing = {**upstream_timing, **timing_breakdown_ms}
        if "total_response_time" in upstream_timing:
            merged_timing["total_response_time_end_to_end"] = upstream_timing.get("total_response_time", 0) + total_time_ms
        return {
            "summary": brief,
            "agents_used": merged_agents,
            "timing_breakdown_ms": merged_timing
        }
    except Exception as e:
